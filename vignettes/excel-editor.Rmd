...
- Correct variable naming mistakes (change aliases)
- Change how variables are displayed (title, description, notes)
- Change category metadata (combine categories, change names, missing-ness, numeric values or dates, order)
- Combine related variables into categorical arrays or multiple response variables
...
- **Organize variables into folders and set their order:** Edit Column B (Folder) to indicate 
  the Folder that the variable is in. Paths with subfolders are separated by "|" between folder names, and "|" is therefore not allowed in folder names.
  ...
  special folder. "|" or blank indicate the root folder. The order within the folders will follow
  the order of the variables in the excel spreadsheet (and in the case of array variables their
...
- Changing a Multiple Response variable's selected categories
- Changing other category metadata for Multiple Response variables
