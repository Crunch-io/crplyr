...
You can now plot crunch variables using ggplot2's `autoplot` method. This will generate a plot which looks a lot like the plots produced on the app, but can be customized with ggplot methods. 
...
ggplot2 has a rich ecosystem of extensions which you can use to add functionality to Crunch plots. For example you can use [patchwork](https://github.com/thomasp85/patchwork) to combine several plots together. 

```{r patchwork, fig.height= 10,  fig.width=8}
library(patchwork)
p1 <- autoplot(ds$Professional)
p2 <- autoplot(ds$TabsSpaces)
p3 <- autoplot(ds$WantWorkMR, "bar")

p3 + {p2 + p1} + plot_layout(ncol = 1)
```

You can also add interactivity to your plots with [plotly](https://plot.ly/r/getting-started/). This allows a user to interact with the plot by clicking and dragging, and lets them select which categories they want to see on the graph. 

```{r plotly,  fig.width=8, fig.height=6}
plotly::ggplotly(autoplot(ds$ImportantHiring))
```
...
cube <- crtabs(~+ CompanySize + Professional  + TabsSpaces, ds)
autoplot(cube)
autoplot(cube, "bar")
```

Autoplots are great, but cubes can quickly become too complex for a general plotting method
to do a good job at representing the plot. If we include lots of dimensions in a cube, or include a dimension with a lot of categories the plot becomes unreadable 

```{r autoplot lots of categories,  fig.width=8, fig.height=6}
cube <- crtabs(~ Country + Professional + TabsSpaces, ds)
autoplot(cube)
```

We can still work with this cube by using the `as_tibble` method to convert it into a long data frame. This lets us further process the data to produce a readable plot.
