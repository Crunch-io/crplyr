...
with_mock_crunch({
    ...
    test_that("summarize makes a cube request", {
        ...
        expect_identical(dim(tbl1), c(1L, 2L))
        expect_identical(names(tbl1), c(".unweighted_counts", "hp"))
        expect_equal(as.numeric(tbl1[1,2]), 146.6875)
    })

    test_that("summarize can handle multiple measures", {
        tbl2 <- summarize(ds, hp=mean(hp), sd_hp=sd(hp), count=n())
        expect_is(tbl2, "tbl_df")
        expect_identical(dim(tbl2), c(1L, 4L))
        expect_identical(names(tbl2), c(".unweighted_counts", "hp", "sd_hp", "count"))
        expect_equal(as.numeric(tbl2[1,2]), 146.6875)
        expect_equal(as.numeric(tbl2[1,4]), 32)
    })
    ...
    test_that("summarize after filter", {
        ...
        expect_identical(dim(tbl3), c(1L, 4L))
        expect_identical(names(tbl3), c(".unweighted_counts", "hp", "sd_hp", "count"))
        expect_equal(as.numeric(tbl3[1,4]), 7)
    })
