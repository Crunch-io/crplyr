...
test_create_single_var_cmd <- function(..., arg = NULL) {
  ...
  cmd <- crunch_auto_cmd(
    formatter = ca_template(
      "{ca_list_to_text(items = in_aliases, sep_newline = FALSE, start_newline = FALSE)} AS {alias}", #nolint
      "{ca_list_to_text('ARG', items = arg, indent = 1, sep_newline = FALSE, start_newline = FALSE)};", #nolint
    ),
...
test_create_multi_var_cmd <- function(..., new_aliases) {
  ...
  cmd <- crunch_auto_cmd(
    formatter = ca_template(
      "{ca_list_to_text(items = in_aliases, sep_newline = FALSE, start_newline = FALSE)} ",
      "{ca_list_to_text('AS', items = new_aliases, sep_newline = FALSE, start_newline = FALSE)};"
    ),
