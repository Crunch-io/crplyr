...
* `select()` (and thus `collect()`) supports taking hidden variables (#13)
* `collect()` now uses `crunch::as.data.frame()` export, which should be faster especially for larger data pulls.
